...
P="/data/xinyua11/robomimic_runs/selection"
...
c.train.data="/data/xinyua11/robomimic_runs/selection/can_mh_sel.hdf5"; c.train.hdf5_filter_key=mask
...
env.reset()
def ev(npz):
    d=np.load(f"{PH}/{npz}"); st,rg=d["states"],d["region"]; su=[]
    for i in range(len(st)):
        policy.start_episode(); ob=env.reset_to({"states":st[i]}); ok=False
        for t in range(HORIZON):
            ob,_,_,_=env.step(policy(ob=ob))
            if env.is_success()["task"]: ok=True; break
        su.append(bool(ok))
    su=np.array(su); Jr={r:float(su[rg==r].mean()) for r in sorted(set(rg.tolist()))}
    return {"J_uniform":float(su.mean()),"J_deploy":float(sum(q[r]*Jr[r] for r in Jr)),"J_region":Jr}
...
print("VAR_EVAL",json.dumps({"cfg":cfg,"seed":seed,"probe":out["probe"]["J_deploy"],"test":out["test"]["J_deploy"]}),flush=True)
