...
P="/data/xinyua11/robomimic_runs/selection"; RES=f"{P}/results.json"
...
while todo or act:
    while todo and len(act)<CONC:
        ...
        pr=subprocess.Popen([PY,f"{P}/run_sel.py",r["name"],r["config"],r["mask"],str(r["seed"])],
    ...
    for n in list(act):
        ...
        if pr.poll() is not None:
            ...
            if pr.returncode==0 and os.path.exists(fe):
                full=json.load(open(fe)); res[n]={k:full[k] for k in ("name","config","mask","seed","probe")}; json.dump(res,open(RES,"w"),indent=1)
                print(f"[pool] DONE {n}: probe={res[n]['probe']['J_deploy']:.3f}",flush=True)
