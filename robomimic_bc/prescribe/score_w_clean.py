...
ckpts = sorted(glob.glob(f"{P}/out/ref_bo_s*/*/models/model_epoch_300*.pth"))
...
json.dump(out, open(f"{P}/w_scores_cleanref.json", "w"), indent=1)
...
json.dump({"pred_worst50": bot50, "pred_best50": top50}, open(f"{P}/gradqual_sets_cleanref.json", "w"))
print("wrote w_scores_cleanref.json + gradqual_sets_cleanref.json")
