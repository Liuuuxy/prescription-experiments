...
**TARGETED-GENERATION COST + TEACHER BIAS (measured 2026-06, generating the targeted pool):** to collect 50 tall-object (height>0.10m) demos with pi0, it took 132 in-band rollouts (50 success / 82 fail = **38% pi0 success on tall objects**, matching [[pi0-weakregion-finding]]) plus 230 rejected short-object resets. So: (1) targeted demos cost **~2.6x more** to generate than random; (2) the collected targeted demos are the *easier* 38% the teacher can do → **systematic bias toward the easy end of the weak region** (the "expert can't teach its own failures" ceiling, now quantified — confirms the algorithm-critique concern). Data pools: `phase1_data/ppc2sink_pi0_baseline100.hdf5` (random/baseline), `phase1_data/ppc2sink_pi0_targeted50.hdf5` (tall-only). See [[expert-data-generation-loop]].
