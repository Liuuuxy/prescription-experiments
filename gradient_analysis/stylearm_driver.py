"""Driver for execution-style arm pulls ("well-defined arms" test, 2026-08-01).

Runs pull.run_pull for arms defined by gradient_analysis/style_assignment.json
...
Env: DRIVER_ARMS="style_hi" DRIVER_ROUNDS="3,4" DRIVER_SLOT=a DRIVER_GPU=0
...
ARMS = [a for a in os.environ.get("DRIVER_ARMS", "style_hi").split(",") if a]
...
B = 200
ASSIGN = "/data/xinyua11/robocasa/gradient_analysis/style_assignment.json"
...
pairs = {}
for name in ("style_hi", "style_lo"):
    for e in assign[name]:
        pairs[int(e)] = name
regions = pd.Series(pairs, dtype=object)
regions.index.name = "episode_index"
log(f"regions built: {regions.value_counts().to_dict()} (features {assign['features']})")
...
for j in ROUNDS:
    for arm in ARMS:
        ...
        log(f"pull start: arm={arm!r} round={j} slot={SLOT} gpu={GPU} "
            f"(execution-style arm, smoke-status)")
...
log(f"STYLEARM DRIVER COMPLETE: arms={ARMS} rounds={ROUNDS}")
